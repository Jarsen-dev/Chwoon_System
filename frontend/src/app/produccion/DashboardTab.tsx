...
export default function DashboardTab({ dashPorParte, dashTotalPiezas, cargarDashboard }: Props) {
  const cards = [
    { label: 'Total Piezas',   value: dashTotalPiezas.toLocaleString(),                              icon: '📦', color: 'border-blue-500   bg-blue-500/10',   text: 'text-blue-400'   },
    { label: 'Partes Activas', value: dashPorParte.length,                                           icon: '🔢', color: 'border-purple-500 bg-purple-500/10', text: 'text-purple-400' },
    { label: 'Completadas',    value: dashPorParte.filter(p => p.porcentaje >= 100).length,          icon: '✅', color: 'border-emerald-500 bg-emerald-500/10', text: 'text-emerald-400' },
    { label: 'En Proceso',     value: dashPorParte.filter(p => p.porcentaje > 0 && p.porcentaje < 100).length, icon: '🔄', color: 'border-yellow-500 bg-yellow-500/10', text: 'text-yellow-400' },
  ]
