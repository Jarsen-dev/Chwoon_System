...
export default function ConfiguracionTab({ token }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <h2 className="text-xl font-bold">⚙️ Configuración de Alertas</h2>

      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Nueva Configuración</h3>
        ...
        <button onClick={handleCrear} disabled={!form.sku} className="mt-3 bg-orange-600 hover:bg-orange-700 disabled:opacity-50 px-4 py-2 rounded-lg text-sm font-medium">Guardar</button>
      </div>

      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Configuraciones Guardadas</h3>
        {loading ? <div className="flex justify-center py-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-orange-400" /></div> : (
            ...
            {configs.map((cfg: any) => (
              <div key={cfg.id} className="flex justify-between items-center bg-gray-800/50 rounded-lg p-3">
                <div className="text-sm">
                  ...
                  <span className="ml-3 text-gray-400">Min: {cfg.stock_minimo}</span>
                  {cfg.stock_maximo && <span className="ml-2 text-gray-400">Max: {cfg.stock_maximo}</span>}
                  {cfg.dias_rotacion && <span className="ml-2 text-gray-400">Rot: {cfg.dias_rotacion}d</span>}
                </div>
                <button onClick={() => eliminarConfigAlerta(token, cfg.id).then(() => cargar()).catch((e: any) => mostrar(e.message, 'err'))} className="bg-red-600/20 text-red-400 px-2 py-1 rounded text-xs">Eliminar</button>
              </div>
            ))}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-red-400 uppercase tracking-wider mb-3">🚨 Alertas Activas ({alertas.length})</h3>
        <div className="space-y-2">
