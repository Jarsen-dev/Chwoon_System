...
export default function DatabaseTab({ token }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h3 className="text-sm font-bold text-gray-300 uppercase tracking-wider mb-1 flex items-center gap-2">
          <IconUbicaciones size={16} aria-hidden /> Almacén — Limpieza de Datos
        </h3>
        <p className="text-xs text-gray-500 mb-4">
          Estas acciones eliminan registros del módulo de Almacén. Son irreversibles.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <DbActionCard
            icon={IconUbicaciones}
            title="Vaciar Ubicaciones"
            description="Elimina TODAS las ubicaciones (racks, silos, zonas). Los lotes que las ocupaban quedan sin ubicar, no se eliminan"
            count={getCount('ubicaciones')}
            buttonLabel="Vaciar Ubicaciones"
            buttonColor="bg-red-600 hover:bg-red-700"
            loading={dbActionLoading === 'vaciar-ubicaciones'}
            onClick={() => confirmarAccionDB(
              'vaciar-ubicaciones',
              'Vaciar Ubicaciones',
